...
/** Recherche instantanée : propose les appareils et mène à leur position sur le plan. */
export default function SearchBox() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const r = await api<DeviceDTO[]>(`/api/devices?q=${encodeURIComponent(term)}&limit=8`).catch(() => []);
  ...
  return (
    <div className="relative">
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (results?.length === 1) router.push(deviceHref(results[0]));
      ...
      {results && (
        <div className="absolute inset-x-0 top-full z-20 mt-1 overflow-hidden rounded-md border border-slate-200 bg-white shadow-lg">
          {results.length === 0 ? (
            <p className="px-4 py-3 text-sm text-slate-500">Aucun appareil trouvé.</p>
          ) : (
            results.map((d) => (
